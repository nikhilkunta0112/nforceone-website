      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-16">
        ...
        <div className="grid md:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {contactData.offices.map((office, idx) => (
            <div key={idx} className="bg-zinc-900 border border-zinc-800 p-6 sm:p-8 rounded-3xl space-y-6 relative overflow-hidden group hover:border-nforce-red/50 transition-all">
              <div className="flex items-center justify-between">
                <div className="space-y-1">
                  <span className="text-xs font-bold text-nforce-red uppercase tracking-widest">{office.badge}</span>
                  <h3 className="text-2xl font-bold text-white">{office.city}, {office.country}</h3>
                </div>
                <div className="w-10 h-10 rounded-xl bg-nforce-red/10 flex items-center justify-center text-nforce-red">
              ...
              <div className="space-y-3 text-xs text-zinc-300">
                <div className="flex items-start space-x-3">
                  <MapPin className="w-4 h-4 text-nforce-red shrink-0 mt-0.5" />
                ...
                <div className="flex items-center space-x-3">
                  <Phone className="w-4 h-4 text-nforce-red shrink-0" />
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {contactData.consultationSteps.map((step, idx) => (
              <div key={idx} className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl space-y-3 relative">
                <div className="text-3xl font-black text-nforce-red">{step.step}</div>
